...
public class Pathfinding : MonoBehaviour {
	...
	void Update() {
		...
		watch2.Stop();
		Debug.Log($"Execution Time A*: {watch1.ElapsedMilliseconds} ms");
		watch2.Start();
		FindPathUCS(seeker.position, target.position);
		watch2.Stop();
		Debug.Log($"Execution Time UCS: {watch2.ElapsedMilliseconds} ms");
		watch3.Start();
		FindPathBFS (seeker.position, target.position);
		watch3.Stop();
		Debug.Log($"Execution Time BFS: {watch3.ElapsedMilliseconds} ms");
		watch4.Start();
		FindPathDFS(seeker.position, target.position);
		watch4.Stop();
		Debug.Log($"Execution Time DFS: {watch4.ElapsedMilliseconds} ms");
